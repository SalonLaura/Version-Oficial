...
class DaysView extends View {
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      /* if(this.picker.datepicker.element.classList.contains("range-today") &&
      this.picker.datepicker.element.classList.contains("children") && 
      (compareDate(new Date(this.picker.datepicker.element.getAttribute("children-date-count")),date) > 11 ||
      compareDate(new Date(this.picker.datepicker.element.getAttribute("children-date-count")),date) < 2)){
          //Aki es donde debo intsertar el codigo para desavilitar los dias de atras
          disableDay = "line-through disabled";
          txtColor = "300";
      }

      else if(this.picker.datepicker.element.classList.contains("range-today") &&
      this.picker.datepicker.element.classList.contains("infant") && 
      compareDate(new Date(this.picker.datepicker.element.getAttribute("children-date-count")),date) > 2){
          //Aki es donde debo intsertar el codigo para desavilitar los dias de atras
          disableDay = "line-through disabled";
          txtColor = "300";
      ...
      else if(date <= new Date() && this.picker.datepicker.element.classList.contains("range-today-up")){
          //Aki es donde debo intsertar el codigo para desavilitar los dias de atras
          disableDay = "line-through disabled";
          txtColor = "300";
      }
      else if(this.picker.datepicker.element.classList.contains("date-return")){
      ...
      el.className = `datepicker-cell hover:bg-gray-100 dark:hover:bg-gray-600 block flex-1 leading-9 border-0 rounded-lg cursor-pointer text-center text-gray-${txtColor} dark:text-white font-semibold text-sm ${this.cellClass} ${disableDay}`;
...
function compareDate(d1,d2){
  const diferenciaEnMilisegundos = d1.getTime() - d2.getTime();
  const milisegundosEnAno = 1000 * 60 * 60 * 24 * 365;
  const diferenciaEnAnos = Math.floor(diferenciaEnMilisegundos / milisegundosEnAno);
  return parseInt(diferenciaEnAnos);
}
...
class Picker {
  constructor(datepicker) {
    ...
    element.id = "datapicker-"+datepicker.element.id;// Aki lo modifique para poder asignarle el id del input
...
function initDatepickers() {
  document.querySelectorAll('[datepicker]').forEach(function (datepickerEl) {
    ...
    //datepickerEl.addEventListener('changeDate', (e) => { e.dateValue})
  });
